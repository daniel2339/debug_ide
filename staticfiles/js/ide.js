...
function loadQuestions(starLevel) {
    let jsonUrl = "{% static 'question.json' %}";
    ...
    fetch(jsonUrl)
        ...
        .then(data => {
            let filteredQuestions = data.filter(q => q.difficulty === starLevel);
            let questionListHtml = "<h4>選擇題目：</h4><ul>";

            filteredQuestions.forEach((question) => {
                questionListHtml += `<li><button onclick="selectQuestion('${question.title}')">${question.title}</button></li>`;
            });
            questionListHtml += "</ul>";
...
// 設定選擇的題目
function selectQuestion(title, content) {
    selectedQuestion = { title: title, content: content };
    alert(`已選擇題目：${title}`);
    document.getElementById("site-content").innerHTML = `<h3>${title}</h3><pre>${content}</pre>`;
}
...
function askOpenAI() {
    ...
    // 檢查是否包含與代碼相關的關鍵詞
    const codeRelatedKeywords = ["剛剛", "代碼", "哪裡有問題", "為什麼這樣改", "debug"];
